...
let getArgumentsWithMultipleJsDocParams = (
  ...
  return _.filter(aModuleArgumentsToCheck, oModuleArgumentToCheck => {
    const aMultipleParams = _.filter(aParamTags, oParamTag => {
      return oParamTag.name === oModuleArgumentToCheck.argName;
    });
    return _.size(aMultipleParams) > 1;
  });
};
...
let reportJsDocParamUndefined = (
  ...
  _.forEach(
    ...
    oArgumentWithMultipleJsDocParams => {
      context.report({
        node: oArgumentWithMultipleJsDocParams.moduleNode,
...
module.exports.meta = {
  type: "suggestion",
  docs: {
    description: "validates jsdoc param exists only once per argument",
